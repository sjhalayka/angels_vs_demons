...
perlin::perlin(mt19937 mt_rand)
{
   int i, j, k;

   for (i = 0 ; i < perlin_B ; i++) {
      p[i] = i;
      g1[i] = (double)((mt_rand() % (perlin_B + perlin_B)) - perlin_B) / perlin_B;

      for (j = 0 ; j < 2 ; j++)
         g2[i][j] = (double)((mt_rand() % (perlin_B + perlin_B)) - perlin_B) / perlin_B;
      normalize2(g2[i]);

      for (j = 0 ; j < 3 ; j++)
         g3[i][j] = (double)((mt_rand() % (perlin_B + perlin_B)) - perlin_B) / perlin_B;
      normalize3(g3[i]);
   }

   while (--i) {
      k = p[i];
      p[i] = p[j = mt_rand() % perlin_B];
      p[j] = k;
   }
